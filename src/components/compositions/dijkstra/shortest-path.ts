...
export const useShortestPath = () => {
  ...
  const setDijkstraVertices = (graph: Graph) => {
    ...
    graph.vertices.forEach(vertex => {
      vertices.push({
        id: vertex.id,
        name: vertex.name,
        edges: graph.edges.filter(edge => edge.sid === vertex.id || edge.tid === vertex.id),
        isSource: false,
        cost: INF,
        previous: null,
        marked: false,
  ...
  const dijkstra = (sId: number) => {
    ...
    let markedVertices = [];
    const source = vertices.find(v => v.id === sId);
    let tr=0, round=1;
    table.innerHTML="";
    ...
    for(let i=0; i< vertices.length; i++){
      ...
      cell.innerHTML+=', '+ ((vertices[i].previous==null)?'-':vertices[i].previous?.name);
    }
    //interface end
    ...
    do {
      ...
      for(let j=0; j< vertices[m].edges.length; j++){
        const neighborId = (vertices[m].edges[j].sid == vertices[m].id)? vertices[m].edges[j].tid : vertices[m].edges[j].sid;
